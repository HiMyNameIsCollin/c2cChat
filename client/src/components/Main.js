...
const Main = ({loggedIn, room, setRoom, userRooms, setUserRooms, socket, tempRooms, height}) => {
	...
	const handleTypeOfRoom = (chatRoomHeight) => {
		...
		if(realRoom !== undefined){
			return <ChatRoom height={height} r={realRoom} loggedIn={loggedIn} socket={socket} roomType={'room'}/>
		} else if (tempRoom !== undefined) {
			return <ChatRoom height={height} r={tempRoom} loggedIn={loggedIn} socket={socket} roomType={'temp'}/>
